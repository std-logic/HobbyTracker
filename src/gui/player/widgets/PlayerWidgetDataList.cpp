...
void Player::WidgetDataList::showByArtists(const Library& library)
{
	...
	for (const auto& [artist_title, artist] : library) {
		...
		item_artist->setHoveredToolTip(artist.summaryString());
		...
		for (const auto& [album_title, album] : artist) {
			...
			item_album->setHoveredToolTip(album.summaryString());
...
void Player::WidgetDataList::showByGenres(const Library& library)
{
	...
	for (const auto& [genre_title, genre_library] : genres) {
		...
		item_genre->setHoveredToolTip(genre_library.summaryString(true));
		...
		for (const auto& [artist_title, artist] : genre_library) {
			...
			for (const auto& [album_title, album] : artist) {
				auto item_album = !misc ?
						new Base::WidgetTreeItem(item_artist, Global::Colors::tree_level_1) :
						new Base::WidgetTreeItem(item_genre, Global::Colors::tree_level_2);
				item_album->setText(CLMN_TITLE, album_title);
				item_album->setText(CLMN_YEAR, album.yearString());
...
void Player::WidgetDataList::showByFormats(const Library& library)
{
	...
	for (const auto& [format_title, format_library] : formats) {
		...
		item_format->setHoveredToolTip(format_library.summaryString());
...
void Player::WidgetDataList::showByYears(const Library& library)
{
	...
	for (const auto& [year_title, year_library] : years) {
		...
		item_year->setHoveredToolTip(year_library.summaryString());
