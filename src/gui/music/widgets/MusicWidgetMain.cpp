...
#include "../data/MusicDataConverter.h"
...
void Music::WidgetMain::initConnections()
{
...
void Music::WidgetMain::showSettings()
{
	if (!_widget_settings) {
...
void Music::WidgetMain::saveSettings(const Settings& settings)
{
	_settings = settings;
	_settings.save();
...
void Music::WidgetMain::readCsvData(const Csv::Settings& csv_settings)
{
...
void Music::WidgetMain::addData()
{
	showData(_data_list.size());
}

void Music::WidgetMain::editData(const QString& id)
{
...
void Music::WidgetMain::showData(size_t index)
{
	if (!_widget_data) {
...
void Music::WidgetMain::deleteData(const QString& id)
{
	if (auto i = _data_list.findIndexById(id); i >= 0) {
		auto ans = QMessageBox::question(this, tr("Удаление данных"),
			tr("Удалить группу \"%1\"?").arg(_data_list[i].title()));
...
void Music::WidgetMain::addExtra()
{
	showExtra(_extra_list.size());
}

void Music::WidgetMain::editExtra(const QString& id)
{
...
void Music::WidgetMain::showExtra(size_t index)
{
	if (!_widget_extra) {
...
void Music::WidgetMain::updateAll()
{
	updateSummary();
	updateDataList();
	updateExtraList();
...
void Music::WidgetMain::updateDependentOnData()
{
	updateSummary();
	updateDataList();
	updateChart();
...
void Music::WidgetMain::updateSummary()
{
	_widget_summary->update(_data_list);
}

void Music::WidgetMain::updateDataList()
{
	_widget_data_list->update(_data_list);
}

void Music::WidgetMain::updateExtraList()
{
	_widget_extra_list->update(_extra_list);
}

void Music::WidgetMain::updateChart()
{
