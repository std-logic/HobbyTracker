...
void Concerts::WidgetMain::showExtra(size_t index)
{
	if (!_widget_extra) {
		...
		_widget_extra->addSpecialGroup(
				tr("[Теги]"),
				tr("Перечисление через запятую тегов,<br>которые ищутся в описаниях концертов"),
				tr("Общее название для концертов с данными тегами")
		);
		_widget_extra->addSpecialGroup(
				tr("[Синонимы для групп]"),
				tr("Перечисление через запятую групп,<br>которые должны объединяться в одну"),
				tr("Общее название для вышеуказанных групп")
		);
		_widget_extra->addSpecialGroup(
				tr("[Синонимы для мест]"),
				tr("Перечисление через запятую мест,<br>которые должны объединяться в одно"),
				tr("Общее название для вышеуказанных мест")
		);
		_widget_extra->addSpecialGroup(
				tr("[Избранные группы]"),
				tr("Если «Только избранные» включены, в режиме «По группам»<br>"
				   "будут отображаться только те, что добавлены здесь"),
				""
		);
		_widget_extra->addSpecialGroup(
				tr("[Избранные места]"),
				tr("Если «Только избранные» включены, в режиме «По местам»<br>"
				   "будут отображаться только те, что добавлены здесь"),
				""
		);
